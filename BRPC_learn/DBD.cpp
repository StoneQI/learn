/*
 * @Author: your name
 * @Date: 2021-02-05 10:29:16
 * @LastEditTime: 2021-02-05 17:46:15
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: /learn/BRPC_learn/DBD.cpp
 */
...
#ifndef DOUBLY_BUFFERED_DATA_H
#define DOUBLY_BUFFERED_DATA_H
...
public:
    explicit lock_guard(pthread_spinlock_t & mutex) : _pmutex(&mutex) {
#if !defined(NDEBUG)
        const int rc = pthread_mutex_lock(_pmutex);
        if (rc) {
            //LOG(FATAL) << "Fail to lock pthread_spinlock_t=" << _pmutex << ", " << berror(rc);
            _pmutex = NULL;
        }
#else
...
private:
    ...
    pthread_spinlock_t _wrappers_mutex;

    // Sequence modifications.
    pthread_spinlock_t _modify_mutex;
};
...
DoublyBufferedData<T, TLS>::DoublyBufferedData()
    ...
    pthread_spin_init(&_modify_mutex, 0);
    pthread_spin_init(&_wrappers_mutex, 0);
...
DoublyBufferedData<T, TLS>::~DoublyBufferedData() {
    ...
    pthread_spin_destroy(&_modify_mutex);
    pthread_spin_destroy(&_wrappers_mutex);
}
...
size_t DoublyBufferedData<T, TLS>::Modify(Fn& fn) {
    ...
    // 修改第二份数据，等待直到所有的数据释放锁，？？读第二个索引时会不会加锁
    {
        BAIDU_SCOPED_LOCK(_wrappers_mutex);
        for (size_t i = 0; i < _wrappers.size(); ++i) {
